...
namespace BitingAxesVR
{
 ...
 if (!user) return {};
 return std::string(user) + "\\Documents\\My Games\\Skyrim VR\\SKSE\\" + kPluginFileSlug + ".log";
 }
 ...
 {
 char title[128];
 snprintf(title, sizeof(title), "%s Fatal Error", kPluginDisplayName);
 MessageBoxA(nullptr, a_errorString, title, MB_ICONERROR | MB_OK | MB_TASKMODAL);
