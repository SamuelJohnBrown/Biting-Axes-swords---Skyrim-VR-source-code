...
namespace BitingAxesVR
{
 ...
 #define IW_LOG_INFO(fmt, ...) do { SKSE::log::info(fmt, ##__VA_ARGS__); BitingAxesVR::AppendToPluginLog("INFO", "%s", std::format(fmt, ##__VA_ARGS__).c_str()); } while(0)
 #define IW_LOG_WARN(fmt, ...) do { SKSE::log::warn(fmt, ##__VA_ARGS__); BitingAxesVR::AppendToPluginLog("WARN", "%s", std::format(fmt, ##__VA_ARGS__).c_str()); } while(0)
 #define IW_LOG_ERROR(fmt, ...) do { SKSE::log::error(fmt, ##__VA_ARGS__); BitingAxesVR::AppendToPluginLog("ERROR", "%s", std::format(fmt, ##__VA_ARGS__).c_str()); } while(0)
